    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(zhaopin !=null){
            ...
            if(gongsi != null){
            BeanUtils.copyProperties( gongsi , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "gongsiId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
...
//                            zhaopinEntity.setGongsiId(Integer.valueOf(data.get(0)));   //用工部门 要改的
        ...
        ZhaopinEntity zhaopin = zhaopinService.selectById(id);
            if(zhaopin !=null){
                ...
                //级联表
                    GongsiEntity gongsi = gongsiService.selectById(zhaopin.getGongsiId());
                if(gongsi != null){
                    BeanUtils.copyProperties( gongsi , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "gongsiId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setGongsiId(gongsi.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
